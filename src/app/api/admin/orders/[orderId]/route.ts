...
export async function PATCH(
  ...
  try {
    ...
    const updatedOrder = await prisma.order.update({
      ...
      include: { 
        user: true,
        paymentChannel: true // Include para obtener el canal de pago
      },
    });
    ...
    if (normalizedStatus === "COMPLETED") {
      ...
      // 🔥 CALCULAR PROFIT REAL usando PaymentChannel
      let profit = 0;
      
      if (updatedOrder.paymentChannel) {
        // Usar el fee del canal de pago
        const providerFeePercent = updatedOrder.paymentChannel.providerFeePercent / 100;
        const afterProviderFee = updatedOrder.amount * (1 - providerFeePercent);
        const sent = updatedOrder.finalUsdt || updatedOrder.finalUsd;
        profit = afterProviderFee - sent;
      } else {
        // Fallback para órdenes viejas sin paymentChannel - asumir PayPal
        const afterPaypal = updatedOrder.amount * 0.955; // 4.5% fee
        const sent = updatedOrder.finalUsdt || updatedOrder.finalUsd;
        profit = afterPaypal - sent;
      }

      // Guardar el profit en la orden
      await prisma.order.update({
        where: { id: orderId },
        data: { profit: profit }
      });

      console.log(`💰 Profit real de esta orden: $${profit.toFixed(2)}`);
    }
